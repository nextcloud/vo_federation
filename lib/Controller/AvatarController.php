...
class AvatarController extends Controller {
	 ...
	 */
	public function getAvatarDark(int $providerId, int $size) {
		return $this->getAvatar($providerId, $size);
	}
	 ...
	 * @return JSONResponse|FileDisplayResponse
	 */
	public function getAvatar(int $providerId, int $size) {
		if ($size <= 64) {
			if ($size !== 64) {
		...
		try {
			...
			$avatar = new NamedAvatar($folder, $this->l, $provider->getIdentifier(), $this->logger, $this->config);
	...
	public function postAvatar(int $providerId): JSONResponse {
	...
	public function deleteAvatar($providerId): JSONResponse {
		try {
			try {
				$providerFolder = $this->appData->getFolder('provider');
			} catch (NotFoundException $e) {
	...
	public function postCroppedAvatar(int $providerId, ?array $crop = null): JSONResponse {
		if (is_null($crop)) {
